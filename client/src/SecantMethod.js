...
import SecantMethod from "./classes/SecantMethod";

const SecantMethodComponent = () => {
  const navigate = useNavigate();

  // สร้าง instance ของ SecantMethod
  const [secant] = useState(() => new SecantMethod("x^2-7"));
  ...
  const inputEquation = (event) => {
    ...
    try {
      secant.setEquation(newEquation);
    } catch (error) {
  ...
  const calculateRoot = () => {
    ...
    if (isNaN(x0num) || isNaN(x1num)) {
      alert("X0 และ X1 ต้องเป็นตัวเลข!");
      return;
    }

    try {
      // ตั้งค่าและคำนวณด้วย OOP method
      secant.setEquation(Equation);
      secant.setInitialValues(x0num, x1num);

      const results = secant.calculate();
      ...
      // ดึงข้อมูลกราฟ
      const chartData = secant.getChartData();
      setXValues(chartData.yValues);
...
export default SecantMethodComponent;
